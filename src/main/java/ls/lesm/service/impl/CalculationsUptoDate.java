...
public class CalculationsUptoDate {
		...
		if (salaryRecords == 1)// number_of_salary_records_of_single_employee==1
		{
			...
			if (Date1.isBefore(toDate) && Date2.isAfter(fromDate)) {
				...
				for (Salary s : salary) {
					...
					profit_or_los = Total_salary_from_client[0]
							- (paid_till_now + totalFinanceExp + allowancesOfEmployee(employeeId));
					...
					expenses.setTotalExpenses(Math.ceil(paid_till_now + allowancesOfEmployee(employeeId)));
			...
			if (Date1.isBefore(toDate) && Date2.isAfter(fromDate)) {
				...
				try {
					...
					profit_or_los = Total_salary_from_client[0]
							- (Total_internal_pay + totalFinanceExp + allowancesOfEmployee(employeeId));
					...
					expenses.setTotalExpenses(Math.ceil(Total_internal_pay + allowancesOfEmployee(employeeId)));
					expenses.setProfitOrLoss(Math.ceil(profit_or_los));
	...
	public double allowancesOfEmployee(int employeeId) {
...
//					}
				...
				if (record.getShiftAllowance() != null) {
					...
					total = total + record.getShiftAllowance();

				}

				if (record.getSpecialAllowance() != null) {

					total = total + record.getSpecialAllowance();
